...
test('workspace confirmation, automatic URL collision, confirmed completion and offline recovery', async () => {
  ...
  try {
    ...
    assert.equal(state.profile.name, 'Nome final', 'required profile is confirmed before entry')
    assert.equal(state.completed, false, 'completion marker remains pending on the server')
