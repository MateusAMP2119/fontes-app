...
export function useOnboardingSync(props: Props) {
  ...
  const signedOutSession = useRef<string | null>(null)
  ...
  useEffect(() => {
    ...
    const load = () => onboardingRequest<{ email: string | null; step: 'email' | 'password' }>('/invitation/registration', { token }).then(entry => {
      ...
      if (!entry.email && !current.current.session) current.current.setDraft(d => ({ ...d, step: 'email' }))
  ...
  useEffect(() => {
    ...
    if (!props.session) {
      ...
      if (previousUser.current) {
        ...
        setInvitationErrors([]); setInviteLink(''); setInvitationFailure(null)
    ...
    const load = async () => {
      ...
      try {
        ...
        if (invite && !result.passwordRequired) {
          ...
          await onboardingRequest('/invitation', { token: invite })
          if (epoch !== generation.current) return
        ...
        if (new URL(location.href).searchParams.has('invite')) {
          ...
          current.current.setDraft(d => ({ ...d, step: 'join', email: session.user.email }))
        }
        if (!state.current || (error instanceof SyncError && [401, 403].includes(error.status))) current.current.onBlocked?.()
        current.current.setError(new URL(location.href).searchParams.has('invite') && error instanceof SyncError ? '' : error instanceof SyncError ? error.message : 'Ambiente indisponível. Nova tentativa em instantes.')
  ...
  async function authenticate(action: () => Promise<{ error?: unknown }>, message: string, field?: string) {
    ...
    try {
      ...
      if (result.error) {
        ...
        current.current.setError(unavailable ? 'Não foi possível concluir o pedido.' : message, unavailable ? undefined : field)
        return false
      }
      return true
  ...
  async function start() {
    ...
    setSendingCode(current.current.draft.step !== 'code')
  ...
  async function google() {
    ...
    try {
      ...
      if (!controller.signal.aborted) current.current.setError(error instanceof GoogleSignInError ? error.message : 'Não foi possível iníciar sessão com a Google.')
    } finally {
  ...
  async function changeEmail(invitedEmail?: string) {
    ...
    if (current.current.session) {
      try {
        const sessionId = current.current.session.session.id
        const result = await authClient.signOut(); if (result.error) throw new Error()
        signedOutSession.current = sessionId
      }
    ...
    clearTimeout(timer.current)
    record.current = { draft: { ...fresh, step: 'email', email: invitedEmail || '' }, revision: 0, invitations: [] }
    setBootstrap(null); setWorkspaceBusy(false); setSyncStatus('idle'); setFailed(false); setIssue(null); setLoading(false); setInvitationErrors([]); setInviteLink(''); setInvitationFailure(null)
    current.current.setDraft(record.current.draft)
    persist()
    return true
  }
