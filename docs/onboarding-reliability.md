...
## Live production invitation check, 13 September 2026

A fresh Google account created the workspace “Teste de convites” and sent an invitation to a different, unregistered email address. The invitation arrived in the real inbox. Its link opened in a separate Safari session; the recipient registered with the emailed code, set a password with user assistance, reviewed and accepted the invitation, and completed profile/preferences setup. The production database confirmed both accounts belonged to the same organization, with owner/member roles and completed onboarding. Reloading Safari restored access without repeating setup.

This fresh production journey passed. It does not establish the cause of the previously reported failures. The separate local invitation-origin fix remains a development change and was not needed for this production run. Test accounts and the workspace were left intact.
