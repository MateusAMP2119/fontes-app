...
export default function Onboarding({ preview = false, session = null, onReady, onBlocked, background = false }: { background?: boolean; preview?: boolean; session?: AuthSession | null; onReady?: (state: Bootstrap) => void; onBlocked?: () => void }) {
  ...
  const live = useOnboardingSync({ preview, session, draft, setDraft, setNotice, setError: toast, onReady, onBlocked })
  ...
  const go = (step: Step) => { if (live.busy) return; patch({ step }); setNotice(''); setError(null) }
  ...
  return <main className="ob-page" data-theme={light ? 'light' : 'dark'}>
    ...
    <div className="ob-stage">
      {brand}
      {live.savingPassword && <p role="status">A guardar palavra-passe. O ambiente já pode ser preenchido.</p>}
      ...
      {!live.restoring && <StepPanel key={draft.step} step={draft.step}>
        ...
        {draft.step === 'start' ? <div className="ob-start-actions">
          ...
          {draft.step === 'password' && <><PasswordField value={password} onChange={setPassword}/><button disabled={!preview && (!session || live.busy || live.loading)} className="ob-button ob-primary ob-wide">{live.savingPassword ? 'A guardar…' : 'Guardar palavra-passe'}</button><button className="ob-subtle" type="button" onClick={() => void live.changeEmail()}>Utilizar um email diferente</button></>}
