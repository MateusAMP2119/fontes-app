...
test('stepper navigates authentication and submits credentials before advancing', async t => {
 ...
 assert.equal(await dot('Palavra-passe').isDisabled(), true)
 await dot('Código').click()
 await p.locator('.ob-panel.ob-code').waitFor()
 await dot('Email').click()
 ...
 assert.equal(await dot('Ambiente').isDisabled(), true)
 await dot('Palavra-passe').click()
 await p.locator('.ob-password').waitFor()
 await p.locator('input[autocomplete="new-password"]').fill('stepper-password-456')
 await dot('Código').click();await p.locator('section.ob-code').waitFor()
 await dot('Palavra-passe').click();await p.locator('.ob-password').waitFor()
 assert.equal(await p.locator('input[autocomplete="new-password"]').inputValue(),'stepper-password-456')
 await dot('Ambiente').click()
 await p.locator('.ob-workspace').waitFor()
 await assertEventually(() => f.state.passwordRequired === false)
 assert.equal(f.calls.filter(call => call.path.endsWith('/sign-in/email-otp')).length, 1)
 assert.equal(f.calls.filter(call => call.path.endsWith('/set-password')).length, 1)
 await dot('Palavra-passe').click()
 await p.locator('.ob-password').waitFor()
 assert.equal(await p.locator('input[autocomplete="new-password"]').inputValue(),'stepper-password-456')
 await dot('Ambiente').click()
 ...
 assert.ok(!stored.includes('stepper-password-456'))
 await dot('Palavra-passe').click()
 await f.button('Utilizar um email diferente').click()
 await p.getByRole('heading',{name:'Registar email',exact:true}).waitFor()
 assert.equal(await f.input('Endereço de email').inputValue(),'')
})
...
test('invitation joins automatically and skips owner setup and owner-only steps',async t=>{
 ...
 assert.equal(await f.button('Ambiente').count(),0);assert.equal(await f.button('Convites').count(),0)
...
test('completed accounts can dismiss an invalid invitation without changing workspace', async t => {
 const f = await fixture(t, {state:{...workspace(),completed:true}}), p=f.page
 await p.route('**/api/onboarding/invitation',route=>route.fulfill({status:403,json:{message:'Convite indisponível.'}}))
 await p.goto(origin+'/?invite='+'ab'.repeat(32))
 await f.button('Tentar ligação novamente').waitFor()
...
test('failed automatic joining pauses old workspace saves and allows retry', async t => {
 const f=await fixture(t,{state:workspace()}),p=f.page
 await p.addInitScript(({user})=>{
  localStorage.setItem('fontes:onboarding:v1:'+user.id,JSON.stringify({
...
for (const authenticated of [false,true]) test(`new invitation starts at password and uses the recipient stepper (sender signed in: ${authenticated})`, async t => {
 ...
 await p.goto(url)
 await p.getByRole('heading',{name:'Definir palavra-passe',exact:true}).waitFor()
 assert.equal(await p.locator('.ob-progress button').count(),3)
 assert.equal(await p.locator('.ob-progress').getAttribute('aria-label'),'Passo 1 de 3')
...
for (const authenticated of [false,true]) test(`password account note and other email restart signup (sender signed in: ${authenticated})`, async t => {
 const f=await fixture(t,{authenticated,state:{...blank(),passwordRequired:true}}),p=f.page,recipient='new-recipient@example.com'
 await p.route('**/api/onboarding/invitation/registration',route=>route.fulfill({json:{email:recipient,step:'password'}}))
 await p.goto(origin+'/?invite='+'bc'.repeat(32))
 await p.getByRole('heading',{name:'Definir palavra-passe',exact:true}).waitFor()
 assert.equal(await p.locator('.ob-password .ob-account-note p').innerText(),`Ambiente para ${recipient}`)
 await p.locator('input[autocomplete="new-password"]').fill('discard-this-password')
 await f.button('Utilizar um email diferente').click()
 await p.getByRole('heading',{name:'Registar email',exact:true}).waitFor()
 assert.equal(await f.input('Endereço de email').inputValue(),'')
 assert.equal(new URL(p.url()).searchParams.has('invite'),false)
 ...
 await p.reload()
 await p.getByRole('heading',{name:'Registar email',exact:true}).waitFor()
 await f.input('Endereço de email').fill('another@example.com');await f.button('Continuar com email').click()
 await f.input(/^Código (?:temporário|de confirmação)$/).fill('123456');await f.button('Continuar com código').click()
 await p.getByRole('heading',{name:'Definir palavra-passe',exact:true}).waitFor()
