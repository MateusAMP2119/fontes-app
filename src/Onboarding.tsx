    ...
    <div className="ob-stage">
      ...
      {!live.restoring && <StepPanel key={draft.step} step={draft.step}>
        ...
        {draft.step === 'start' ? <div className="ob-start-actions">
          ...
          {draft.step === 'password' && <><PasswordField value={password} onChange={setPassword}/><button disabled={live.busy || live.loading} className="ob-button ob-primary ob-wide">{live.busy ? 'A guardar…' : 'Guardar palavra-passe'}</button><button className="ob-subtle" type="button" onClick={() => void live.changeEmail()}>Utilizar um email diferente</button></>}
