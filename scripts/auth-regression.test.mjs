...
test('verified authentication, confirmed sync and offline recovery', async () => {
 ...
 try{
  ...
  await button('Continuar com email').click();await input('Endereço de email').fill(user.email);await button('Continuar com email').click();await input('Código de confirmação').fill('123456');await button('Continuar com código').click();await input('Nome').waitFor();assert.equal(authenticated,true);assert.equal(writes.length,0);
