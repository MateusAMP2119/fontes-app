...
export default function Onboarding({ preview = false, session = null, onReady }: { preview?: boolean; session?: AuthSession | null; onReady?: (state: Bootstrap) => void }) {
  ...
  return <main className="ob-page" data-theme={light ? 'light' : 'dark'}>
    ...
    {error && <div className="ob-toast" role="alert" key={error.n}><span className="ob-toast-dot" aria-hidden="true"/><span>{error.text}</span><button type="button" aria-label="Fechar" onClick={() => setError(null)}><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" aria-hidden="true"><path d="M6 6l12 12M18 6 6 18"/></svg></button></div>}
