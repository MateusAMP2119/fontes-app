...
const fresh: Draft = { step: 'start', returning: false, provider: 'email', email: '', name: '', slug: '', profile: '', image: '', invitations: '', changelog: false, daily: false }
...
export default function Onboarding({ preview = false, session = null, onReady }: { preview?: boolean; session?: AuthSession | null; onReady?: (state: Bootstrap) => void }) {
  ...
  async function pickImage(event: ChangeEvent<HTMLInputElement>) {
    ...
    event.target.value = ''
    if (!file) return
    if (!file.type.startsWith('image/') || file.size > 12_000_000) { toast('Escolhe uma imagem de até 12 MB.'); return }
    try {
  ...
  const required: Partial<Record<Step, string>> = { email: 'Introduz um endereço de email válido.', code: 'Introduz o código de seis dígitos enviado por email.', workspace: 'Dá um nome ao ambiente de trabalho.', profile: 'Escolhe um nome de perfil.' }
  function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault()
    const invalid = event.currentTarget.querySelector<HTMLInputElement>(':invalid')
    if (invalid) { toast(invalid.title || required[draft.step] || 'Preenche os campos em falta.'); invalid.focus(); return }
    ...
    if (draft.step === 'invites') {
      ...
      if (addresses.some(value => !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value))) { toast('Confirma os endereços de email dos convites.'); return }
  ...
  return <main className="ob-page" data-theme={light ? 'light' : 'dark'}>
    <div className="ob-stage">
      ...
      <section className={`ob-panel ob-${draft.step}`} aria-labelledby="ob-title" key={draft.step}>
        ...
        {draft.step === 'start' ? <div className="ob-start-actions">
          ...
          {draft.step === 'email' && <><input aria-label="Endereço de email" type="email" autoComplete="email" placeholder="Endereço de email" required maxLength={254} value={draft.email} onChange={e => patch({ email: e.target.value })}/><button disabled={live.busy} className="ob-button ob-primary ob-wide">{live.busy ? 'A enviar…' : 'Continuar com email'}</button><button className="ob-subtle ob-centered" type="button" onClick={() => go('start')}>Voltar ao início</button></>}
          {draft.step === 'code' && <><input className="ob-code" aria-label="Código de confirmação" inputMode="numeric" autoComplete="one-time-code" placeholder="Introduzir código" pattern="[0-9]{6}" maxLength={6} required value={code} onChange={e => setCode(e.target.value.replace(/\D/g, '').slice(0, 6))}/><button className="ob-button ob-primary ob-wide">Continuar com código</button><button type="button" className="ob-subtle ob-resend" disabled={live.busy} onClick={() => preview ? setNotice('Novo código enviado. Nesta pré-visualização, utiliza quaisquer seis dígitos.') : void live.start()}>Reenviar código</button><button type="button" className="ob-subtle ob-centered" onClick={() => go('start')}>Voltar ao início</button></>}
          {draft.step === 'workspace' && <><Field label="Nome"><input placeholder="Nome do ambiente de trabalho" autoComplete="organization" required maxLength={80} value={draft.name} onChange={e => patch({ name: e.target.value, slug: draft.slug === slug(draft.name) ? slug(e.target.value) : draft.slug })}/></Field><Field label="URL"><div className="ob-url"><span>fontes.app/</span><input aria-label="URL do ambiente" placeholder="a-tua-equipa" pattern="[a-z0-9][a-z0-9\-]{2,47}" title="Entre 3 e 48 letras minúsculas, números ou hífenes." required value={draft.slug} onChange={e => patch({ slug: e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '') })}/></div></Field><button className="ob-button ob-primary ob-wide">Criar ambiente</button><div className="ob-account-note"><p>Ambiente para <span>{email}</span></p><button type="button" className="ob-subtle" onClick={() => { if (!preview) { void live.changeEmail(); return }; patch({ returning: false, provider: 'email' }); go('email') }}>Utilizar um email diferente</button></div></>}
          ...
          {progress >= 0 && <div className="ob-progress" aria-label={`Passo ${progress + 1} de ${flow.length}`}>{Array.from({ length: flow.length }, (_, i) => <span key={i} className={i === progress ? 'current' : i < progress ? 'past' : ''}/>)}</div>}
    ...
    {error && <div className="ob-toast" role="alert" key={error.n}><span>{error.text}</span><button type="button" aria-label="Fechar" onClick={() => setError(null)}><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" aria-hidden="true"><path d="M6 6l12 12M18 6 6 18"/></svg></button></div>}
